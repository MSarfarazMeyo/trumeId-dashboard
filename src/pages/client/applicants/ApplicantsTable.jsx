...
import { useFlowsQuery } from "../../../hooks/useFlows";
...
const ApplicantsTable = () => {
  ...
  const { data: FlowData } = useFlowsQuery({});

  const [isManualModalOpen, setIsManualModalOpen] = useState(hasNewApplicant);
  ...
  const [showFilterOptions, setShowFilterOptions] = useState(!!flowId);
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 120,
      render: (_, record) => (
        <span className="text-gray-600">{record.documentType || "-"}</span>
      ),
    },
    {
      title: "Verification Steps",
      dataIndex: "verificationSteps",
      key: "verificationSteps",
      width: 200,
      render: (_, record) => {
        const verificationTypes = (record?.requiredVerifications || [])
          .slice()
          .sort((a, b) => {
            return (
          ...
          <div className="flex space-x-2">
            {verificationTypes.length &&
              verificationTypes.map((item) => {
                return getVerificationIcons(item.verificationType, item.status);
              })}
          </div>
        );
      },
          ...
          onClick={() => {
            const newOrder =
              filters.sortBy === "createdAt" && filters.sortOrder === "asc"
                ? "desc"
                : "asc";
            handleSortChange(`createdAt-${newOrder}`);
          }}
        >
  ...
  console.log("filters.verificationStatus", filters.verificationStatus);
    ...
    <div className="p-16 bg-gray-50 min-h-screen">
          ...
          <Row gutter={[16, 16]} className="mb-4">
            ...
            <Col span={6}>
              <Select
                ...
                onChange={(value) => handleFilterChange("flowId", value)}
                allowClear
                style={{ width: "100%" }}
              >
      ...
      {isManualModalOpen && (
        <NewApplicantModal
          ...
          setIsModalOpen={setIsManualModalOpen}
        />
      )}
    </div>
  );
